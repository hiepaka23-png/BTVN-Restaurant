...
// - Lúc dev (`ng serve`): frontend và backend là 2 tiến trình khác nhau (backend luôn ở cổng
//   3000) — suy ra hostname từ trang đang mở (thay vì hardcode "localhost") để mở được từ
...
//   "Failed to fetch". Dùng isDevMode() (dựa vào bản build dev/production, cố định lúc build,
//   không đổi theo cổng) để nhận diện — cổng 4200 chỉ là mặc định, không phải lúc nào cũng đúng
//   (vd cổng đó đang bị chiếm, ng serve tự đổi sang cổng khác).
...
export const BACKEND_ORIGIN = isDevMode()
